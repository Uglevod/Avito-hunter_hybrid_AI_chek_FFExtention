...
class PopupController {
  ...
  async loadSystemPrompt() {
    try {
      const response = await fetch(browser.runtime.getURL('sysPrompt'));
      const promptText = await response.text();
      this.systemPromptInput.value = promptText.trim();
      console.log('Popup: Загружен системный промпт из файла');
    } catch (error) {
      console.error('Ошибка загрузки системного промпта:', error);
      this.systemPromptInput.value = 'Анализируй объявления и отвечай только в формате {"good": true} или {"good": false}.';
    }
  }
  ...
  async savePrompt() {
    try {
      await this.saveSettings();
      this.showStatus('Промпт сохранен!', 'success');
    } catch (error) {
      console.error('Ошибка сохранения промпта:', error);
      this.showStatus('Ошибка сохранения промпта', 'error');
    }
  }
